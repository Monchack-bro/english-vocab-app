...
class VocabularyApp {
  ...
  updateCard() {
    ...
    this.cardContainer.innerHTML = `
      <div class="card relative min-h-[120px] cursor-pointer" style="perspective: 1000px; transform-style: preserve-3d; transition: transform 0.6s;">
        <div class="card-face card-front absolute w-full h-full bg-white rounded-lg shadow-md p-6" style="backface-visibility: hidden; transform: ${this.isFlipped ? 'rotateY(180deg)' : 'rotateY(0deg)'};">
          <div>
            <div class="flex items-center justify-center w-full h-16 overflow-hidden">
  <div class="text-2xl font-bold w-full text-left px-2 whitespace-normal card-english-word" style="max-width: 100%; overflow-wrap: break-word; word-break: break-all; display: block;">${card.english}</div>
</div>
            <div class="mt-2 text-sm text-gray-500 text-center">クリックして反転</div>
          </div>
        </div>
        <div class="card-face card-back absolute w-full h-full bg-gray-100 rounded-lg shadow-md p-6" style="backface-visibility: hidden; transform: rotateY(180deg) ${this.isFlipped ? 'rotateY(180deg)' : ''};">
          <div>
            <div class="text-2xl font-bold w-full text-left break-words whitespace-normal px-2">${card.japanese}</div>
